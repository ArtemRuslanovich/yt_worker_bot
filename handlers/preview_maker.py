...
async def view_tasks(message: types.Message, state: FSMContext):
    ...
    if tasks:
        ...
        await AuthStates.preview_maker.set()
    else:
...
def register_handlers(dp: Dispatcher):
    dp.register_message_handler(view_tasks, commands=['my_tasks'], state=AuthStates.preview_maker)
    dp.register_callback_query_handler(select_task, lambda c: c.data.startswith('select_task'), state=AuthStates.preview_maker)
    dp.register_callback_query_handler(send_task_for_review, lambda c: c.data == 'send_review', state=AuthStates.preview_maker)
