...
def register_handlers(dp: Dispatcher):
    dp.register_message_handler(create_story, commands=['create_story'], state=AuthStates.moderator)
    dp.register_message_handler(handle_new_story, content_types=types.ContentTypes.TEXT, state=AuthStates.moderator)
    dp.register_message_handler(view_tasks, commands=['view_tasks'], state=AuthStates.moderator)
    dp.register_callback_query_handler(review_task, lambda c: c.data.startswith('review_task'), state=AuthStates.moderator)
    dp.register_callback_query_handler(approve_task, lambda c: c.data == 'approve_task', state=AuthStates.moderator)
    dp.register_callback_query_handler(revise_task, lambda c: c.data == 'revise_task', state=AuthStates.moderator)